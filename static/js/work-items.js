...
window.loadWorkItems = async function(content) {
    ...
    content.innerHTML = `
        <section class="work-items-shell">
            <div class="work-items-head"><div><h2><i class="fas fa-list-check"></i> 中心工作事项</h2><p>一件工作、一组步骤、一个明确的下一步与催办日期。</p></div><button class="btn btn-primary" onclick="window.openWorkItemForm()"><i class="fas fa-plus"></i> 新建中心事项</button></div>
