...
- 完成第一轮减法重构：主导航收敛为四个高频入口，低频模块进入「更多工具」；首页仅保留行动、中心风险和事项催办。
- 中心工作事项新增 Excel 模板下载、导入校验和导出；导入仅匹配既有项目/中心，并自动同步当前步骤到待办。
- 新增中心风险分级：自动汇总逾期待办、重大/逾期问题、事项催办、伦理逾期和培训需跟进，生成低/中/高风险及可读原因；首页和中心概览使用同一规则。
- 项目驾驶舱「重点中心」已改为复用统一中心风险分级，并展示风险原因。
- 中心详情概览新增「当前行动」队列，优先呈现中心级逾期、今日、执行中待办及未同步的事项催办；风险判断同步纳入中心事项催办。
- Dashboard「优先处理」收敛为按风险排序的「今日三件事」，将逾期、今日、执行中及本周高优先级待办放在同一行动序列；中心事项同步任务也纳入其中。
- 新增中心工作事项与待办联动：每个事项当前未完成步骤会同步为一条系统待办；完成待办后自动回写步骤并推进下一步。手工待办仍可独立维护。
- 新增 `supabase/center_work_item_task_link.sql`，为待办增加来源和中心事项/步骤关联字段。
...
- 待执行 `supabase/center_work_item_task_link.sql` 后启用中心事项与待办自动同步。
